...
inline static void destroy_lock(lock* l)
{
}

inline static void get_lock(lock* l)
{
}
...
{
	sifting_workspace* w = (sifting_workspace*) malloc(sizeof(sifting_workspace));
	w->N = N;
	w->maxx = (double*) malloc(N * sizeof(double));
	w->maxy = (double*) malloc(N * sizeof(double));
	w->minx = (double*) malloc(N * sizeof(double));
	w->miny = (double*) malloc(N * sizeof(double));
	w->maxspline = (double*) malloc(N * sizeof(double));
	w->minspline = (double*) malloc(N * sizeof(double));
	...
	w->spline_workspace = (double*) malloc(spline_workspace_size * sizeof(double));
	return w;
}

void free_sifting_workspace(sifting_workspace* w)
{
	free(w->spline_workspace);
	w->spline_workspace = NULL;
	free(w->minspline);
	w->minspline = NULL;
	free(w->maxspline);
	w->maxspline = NULL;
...
{
	emd_workspace* w = (emd_workspace*) malloc(sizeof(emd_workspace));
	w->N = N;
	w->res = (double*) malloc(N * sizeof(double));
...
{
	eemd_workspace* w = (eemd_workspace*) malloc(sizeof(eemd_workspace));
	w->N = N;
...
static inline libeemd_error_code _validate_eemd_parameters(unsigned int ensemble_size, double noise_strength,
                                                           unsigned int S_number, unsigned int num_siftings);
                        ...
                        S_number, unsigned int num_siftings, unsigned long int rng_seed)
{
	...
	// Validate parameters
	libeemd_error_code validation_result =
		_validate_eemd_parameters(ensemble_size, noise_strength, S_number, num_siftings);
...
#pragma omp single
		{
			ws = (eemd_workspace**)malloc(num_threads * sizeof(eemd_workspace*));
			locks = (lock**) malloc(M * sizeof(lock*));
			for (size_t i = 0; i < M; i++)
			{
				locks[i] = (lock*) malloc(sizeof(lock));
				init_lock(locks[i]);
			}
		}
                           ...
                           S_number, unsigned int num_siftings, unsigned long int rng_seed)
{
	...
	lock* output_lock = (lock*) malloc(sizeof(lock));
	init_lock(output_lock);
	// The threads also share the same precomputed noise
	double* noises = (double*) malloc(ensemble_size * N * sizeof(double));
	// Since we need to decompose this noise by EMD, we also need arrays for storing
	// the residuals
	double* noise_residuals = (double*)malloc(ensemble_size * N * sizeof(double));
	// Don't start unnecessary threads if the ensemble is small
#ifdef _OPENMP
	if (omp_get_num_threads() > (int)ensemble_size) {
		omp_set_num_threads(ensemble_size);
...
#pragma omp flush(sift_err)
	...
	free(res);
	res = NULL;
	free(noise_residuals);
	noise_residuals = NULL;
	free(noises);
	noises = NULL;
	...
	*nzc = 0;
	...
	for (size_t i = 0; i < N - 1; i++)
	{
		if (x[i + 1] > x[i])
		{
			...
			if (previous_sign == NEG && x[i + 1] > 0)
			{
				// zero crossing from neg to pos
				(*nzc)++;
				previous_sign = POS;
			}
		...
		else if (x[i + 1] < x[i])
		{
			...
			if (previous_sign == POS && x[i + 1] < -0)
			{
				// zero crossing from pos to neg
				(*nzc)++;
				previous_sign = NEG;
			}
			else if (previous_sign == ZERO && x[i + 1] < -0)
